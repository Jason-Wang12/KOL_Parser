...
'''
!!!!TESTING!!!!
Trying to see if it's worth checking over first and/or last names of emails 
before returning, think it might not be worth it
'''
'''
print(emails['Last'].apply(lambda x: x['check'] in x['email']))
def check_name(emails):
    for index, row in emails.iterrows():
        if emails['Last'].apply(lambda x: x['check'] in x['email']) is True:
            return emails['email']
        else: 
            return 'na'

if __name__ == '__main__':
    emails = check_name(emails)

'''

'''
Rank authors based on number of times cited recently
'''

authors = pd.DataFrame(data = record_list, columns = ['FAU', 'PMID'])
#pmid = ["31150574",'28578607']

def citations(authors):
...
def count(pmc_ids):
    for i in results:
       cites = [link["Id"] for link in results[i]["LinkSetDb"][0]["Link"]] 
       authors['citations'] = len(cites(i))
    return authors
        
    

if __name__ == '__main__':
    results = citations(authors)
    count = count(results)
...
pmid = ["31150574", '28578607']
results2 = Entrez.read(Entrez.elink(dbfrom="pubmed", db="pmc",
                                   LinkName="pubmed_pmc_refs", 
                                   id=pmid))
...
df = pd.DataFrame.from_dict(data = results2)
rename__
df = df
df['count'] = df['LinkSetDb'].apply(lambda x: x["Id"] for x in results2[x]["LinkSetDb"][0]["Link"]])
df.to_excel('C:/Users/jwang/Desktop/Python/Biopython/CSV/test.xlsx')


asdf = [link["Id"] for link in results2[1]["LinkSetDb"][0]["Link"]] 
print(len(asdf))
